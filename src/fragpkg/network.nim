import nativesockets, net, openssl,
       bitstream

when defined(windows):
  proc wsaCleanup*(): int32 {.
    stdcall, importc: "WSACleanup", dynlib: "Ws2_32.dll".}
...
template createCommand(c: static[string]): uint32 =
  (uint32(c[0]) shl 0) or
    (uint32(c[1]) shl 8) or
    (uint32(c[2]) shl 16) or
    (uint32(c[3]) shl 24)

var
  cmdHello: uint32 = createCommand("HELO")
  cmdPoop: uint32 = createCommand("POOP")

proc `=destroy`*(fn: var FragNetwork) =
    when defined windows:
      # TODO: Handle this better - https://docs.microsoft.com/en-us/windows/win32/api/winsock/nf-winsock-wsacleanup
      assert wsaCleanup() == 0
    fn.socket.close()

proc newFragNetwork*(): FragNetwork =
  result  

proc init*(fn: var FragNetwork; host: string; port: uint32 = 0) =
  let 
    handle = createNativeSocket()
    ctx = newContext(verifyMode = CVerifyPeer)
  
  fn.socket = newSocket(handle)
  discard SSL_CTX_load_verify_locations(ctx.context, "H:\\Projects\\FRAG\\src\\publicKey.pem", "")
  ctx.wrapSocket(fn.socket)
  fn.socket.connect("127.0.0.1", 4443.Port)
  handle.setBlocking(false)

  var helloPacket = newOutputMemoryBitStream()
  helloPacket.write(cmdHello)
  helloPacket.write(cmdPoop)

  echo repr helloPacket.getBufferPtr()
  echo helloPacket.getByteLength()

  echo fn.socket.send(helloPacket.getBufferPtr(), helloPacket.getByteLength().int)
